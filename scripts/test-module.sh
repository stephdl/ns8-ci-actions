...
# Vendored verbatim from NethServer/ns8-github-actions@v1:scripts/test-module.sh.
# Keep it byte-identical: a module suite has to see the same runner whether it is
# tested on DigitalOcean or on the QEMU node, so re-sync rather than patch here.
